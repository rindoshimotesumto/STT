  const dropzone = document.getElementById("dropzone");
      ...
      const translations = {
        ...
        ru: {
          ...
          recordingOutput: "Запись сохранена. Нажмите «Начать», чтобы распознать.",
          ...
          outputReady: "Нажмите «Начать», чтобы запустить распознавание.",
          checkingText: "Проверяю длительность файла...",
        ...
        uz: {
          ...
          recordingOutput: "Yozuv saqlandi. Tanish uchun «Boshlash» tugmasini bosing.",
          ...
          outputReady: "Tanishni boshlash uchun «Boshlash» tugmasini bosing.",
      ...
      function applyStatus(text, type = "") {
        ...
        transcribeBtn.classList.toggle("action-error", type === "error");
        transcribeBtn.classList.toggle("action-recording", type === "recording");
      }
      ...
      function setAudioPreview(file, knownDuration = null) {
        ...
        audioPlayer.pause();
        previewKnownDuration = Number.isFinite(knownDuration) ? knownDuration : null;
      ...
      async function handleFile(file) {
        ...
        fileSize.textContent = `${formatBytes(file.size)}${Number.isFinite(duration) ? ` · ${Math.round(duration)}s` : ""}`;
      ...
      async function startRecording() {
        try {
          ...
          mediaRecorder.addEventListener("stop", () => {
            ...
            const duration = Math.min(maxAudioDurationSeconds, Math.max(1, Math.round(recordedDurationSeconds)));
            const file = new File([blob], `${getTimestampFileName()}.${extension}`, { type: finalMimeType });
            ...
            setStatusKey("recordedReady");
            setOutputKey("recordingOutput", true);
            mediaRecorder = null;
          });
      ...
      const demoTranscriptionTexts = {
        en: [
          "Hello. This is a test transcription for the uploaded audio.",
          "The audio was received successfully. Speech recognition result will appear here.",
          "This is a demo text. Connect /api/stt to show real transcription."
        ],
        ru: [
          "Здравствуйте. Это тестовая расшифровка загруженного аудио.",
          "Аудио успешно получено. Здесь будет отображаться результат распознавания.",
          "Это демо-текст. Подключите /api/stt, чтобы получать реальную расшифровку."
        ],
        uz: [
          "Salom. Bu yuklangan audio uchun test transkripsiya matni.",
          "Audio muvaffaqiyatli qabul qilindi. Tanish natijasi shu yerda chiqadi.",
          "Bu demo matn. Haqiqiy transkripsiya uchun /api/stt endpointini ulang."
        ]
      };

      function getRandomDemoText() {
        const list = demoTranscriptionTexts[currentLang] || demoTranscriptionTexts.en;
      ...
      function downloadSelectedAudio() {
        ...
        link.href = url;
        link.download = selectedFile.name || `${getTimestampFileName()}.webm`;
      ...
      transcribeBtn.addEventListener("click", async () => {
        ...
        try {
          ...
          if (!response.ok) {
            const errorMessage = data?.detail || data?.message || t("failedText");
          ...
          setOutput(error?.message || t("failedText"), true);
          transcribeBtn.disabled = false;
        }
      });
